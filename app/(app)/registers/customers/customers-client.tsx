...
const stCfg: Record<string, { label: string; c: string; bg: string }> = {
  ...
  bloqueado:   { label:"Bloqueado",   c:"var(--danger)",  bg:"var(--danger-soft)" },
}
...
export default function CustomersClient({ customers }: { customers: Customer[] }) {
  ...
  const filtered = customers.filter(c =>
    ...
    (c.document ?? "").toLowerCase().includes(q.toLowerCase())
  )
  ...
  return (
    <div style={{ padding:"22px" }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"10px",marginBottom:"16px" }}>
        {[
          { l:"Total Clientes",    v:String(customers.length), c:"var(--accent)" },
          { l:"Total Faturado",    v:"—", c:"var(--success)" },
          { l:"Em Atraso",         v:"—", c:"var(--danger)" },
          { l:"Taxa Inadimplência",v:"—", c:"var(--danger)" },
        ].map(k=>(
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ background:"var(--bg-secondary)",border:"1px solid var(--accent-border)",borderRadius:"var(--radius)",padding:"20px",marginBottom:"16px" }}>
          ...
          <div style={{ display:"flex",gap:"8px",marginTop:"14px",paddingTop:"14px",borderTop:"1px solid var(--border)" }}>
            <button type="submit" disabled={saving} style={{ padding:"9px 20px",background:"var(--accent)",border:"none",borderRadius:"6px",fontSize:"12px",color:"#fff",fontWeight:700,cursor:saving?"default":"pointer",opacity:saving?0.7:1,fontFamily:"inherit" }}>{saving ? "Salvando..." : "Salvar cliente"}</button>
      ...
      <div style={{ background:"var(--bg-secondary)",border:"1px solid var(--border)",borderRadius:"var(--radius)",overflow:"hidden" }}>
        <table style={{ width:"100%",borderCollapse:"collapse" }}>
          ...
          <tbody>
            {filtered.map((c,i)=>{
              const sc = stCfg[c.status] ?? { label:c.status, c:"var(--text-muted)", bg:"var(--bg-tertiary)" }
              return (
                <tr key={c.id} style={{ borderBottom:i<filtered.length-1?"1px solid var(--border)":"none" }}
                  ...
                  <td style={{ padding:"12px 14px",fontSize:"11px",color:"var(--text-secondary)" }}>{c.document ?? "—"}</td>
                  ...
                  <td style={{ padding:"12px 14px",textAlign:"right",fontSize:"12px",fontWeight:700,color:"var(--text-muted)" }}>—</td>
                  <td style={{ padding:"12px 14px",fontSize:"11.5px",color:"var(--text-secondary)",textAlign:"center" }}>—</td>
                  <td style={{ padding:"12px 14px" }}>
                  ...
                  <td style={{ padding:"12px 14px" }}>
                    <div style={{ display:"flex",gap:"5px" }}>
                      <button type="button" title="Histórico" style={{ width:"26px",height:"26px",borderRadius:"5px",border:"1px solid var(--border)",background:"var(--bg-elevated)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"var(--text-secondary)" }}><Eye size={12}/></button>
